		...
		<el-table :data="tData" @current-change="onCurrentChange" :expand-row-keys="expandRowKeys" row-key="id" highlight-current-row style="width: 100%">
			<el-table-column type="expand">
				<template slot-scope="props">
					<p>链接：<a :href="props.row.link" target="_blank">{{props.row.link}}</a> <span v-if="props.row.secret" style="margin-left: 20px;">提取码：{{props.row.secret}}</span></p>
				</template>
			</el-table-column>
			<el-table-column label="分享文件" prop="name">
			</el-table-column>
	...
	methods: {
		listRefresh() {
			...
			this.$axios.get('/api/shares').then(ret => {
				this.tData = ret.data.list.map(item => { item.link = `http://${host}/s/${item.alias}`; return item })
			})
		},
